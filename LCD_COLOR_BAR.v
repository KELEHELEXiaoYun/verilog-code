module lcd_drive (
    ...
    always @(posedge lcd_pclk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if ((cnt_h > h_sync + h_back - 1) && (cnt_h < h_sync + h_back + h_disp) &&
                (cnt_v > v_sync + v_back - 1) && (cnt_v < v_sync + v_back + v_disp)) begin
                lcd_de <= 1'b1;
            end else begin
                lcd_de <= 1'b0;
            end
    ...
    always @(posedge lcd_pclk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if ((cnt_h > h_sync + h_back - 2) && (cnt_h < h_sync + h_back + h_disp - 1) &&
                (cnt_v > v_sync + v_back - 1) && (cnt_v < v_sync + v_back + v_disp)) begin
                data_req <= 1'b1;
            end else begin
                data_req <= 1'b0;
...
module lcd_rgb_colorbar (
    ...
    genvar i;
    generate
        for (i = 0; i < 24; i = i + 1) begin : IOBUF_LOOP
            IOBUF #(
